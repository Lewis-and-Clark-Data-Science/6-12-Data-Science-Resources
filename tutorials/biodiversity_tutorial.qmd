...
:::
::: {.column width="65%"}
![from.https://www.newscientist.com/definition/biodiversity/](https://lewis-and-clark-data-science.github.io/6-12-Data-Science-Resources/files/figures/biodiversityimage.jpg){cap-location="margin" height=3.5in}
:::
:::
...
We are going to be using data from [BioTIME](https://biotime.st-andrews.ac.uk){target="_blank"}
...
-   How might biodiversity index and number of different species represent biodiversity in an area? 
...
(Go back to look at the [`Biodiversity_Index`](#sec-dataset) data set to try to find the answer)
...
Let's use R to solve this question instead! The `unique()` function lists unique values within a column, so it shows us all the different values.
