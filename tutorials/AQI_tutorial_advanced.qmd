...
**Replace** the current numbers with your AQI value.
...
-   How do you think the EPA makes those decisions? (Check out [table 6 in this document](https://document.airnow.gov/technical-assistance-document-for-the-reporting-of-daily-air-quailty.pdf) if you want more information on this!)

-   Do you think the same AQI level affects everyone equally? Or does it affect some people more than others? 
...
myCountyData<- usa_aqi_data|>
  ...
  ggplot(myCountyData, aes_string(x = xvar, y = "Year", fill = "AQI_Category"))+
  geom_tile()+
  scale_y_reverse()+
  scale_fill_manual(values=aqi_colors, drop=FALSE)+
